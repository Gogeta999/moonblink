...
class UserTransactionBloc
  ...
  @override
  Stream<Transition<UserTransactionEvent, UserTransactionState>>
      transformEvents(Stream<UserTransactionEvent> events, transitionFn) {
  ...
  Stream<UserTransactionState> mapEventToState(
    ...
    if (event is UserTransactionFetched && !_hasReachedMax(currentState)) {
      if (currentState is UserTransactionInitial) {
        ...
        try {
          data = await _fetchUserTransaction(limit: transactionLimit, page: 1);
        } catch (_) {
          yield UserTransactionNoData();
        }
        bool hasReachedMax = data.length < transactionLimit ? true : false;
        yield UserTransactionSuccess(
  ...
  Stream<UserTransactionState> _mapRefreshedToState(
      UserTransactionState currentState) async* {
    List<Transaction> data = [];
  ...
  Future<List<Transaction>> _fetchUserTransaction({int limit, int page}) async {
    UserTransaction userTransaction =
        await MoonBlinkRepository.getUserTransaction(limit: limit, page: page);
